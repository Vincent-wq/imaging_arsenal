...
- Rerun list with subjects that need to be rerun (no header, columns: participant_id, session)
"""
...
def audit_subject(roots: Roots, sub: str) -> Dict:
    ...
    anat_dir = deriv_sub / "ses-0" / "anat"
    func_dir = deriv_sub / "ses-0" / "func"
    ...
    if func_dir.exists():
        preproc_bolds = sorted(func_dir.glob(f"sub-{sub}*_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii*"))
    ...
    # and (n_preproc_bold > 0) and (missing_confounds_for_runs == 0)
    # report+T1+fMRI
    # ok_min = report_html.exists() and (anat_preproc_t1w is not None) and (n_preproc_bold > 0)
    # T1+fMRI
    ok_min = (anat_preproc_t1w is not None) and (n_preproc_bold > 0)
...
def generate_rerun_list(df: pd.DataFrame, rerun_path: Path) -> Path:
    """
    Generate a rerun list from the audit table.
    
    Parameters
    ----------
    df : pd.DataFrame
        The audit table DataFrame
    audit_table_path : Path
        Path to the audit table output
    
    Returns
    -------
    Path
        Path to the generated rerun list file
    """
    # Filter subjects with status != "OK"
    rerun_df = df[df["status"] != "OK"].copy()
    
    if rerun_df.empty:
        print("No subjects need to be rerun.")
        return None
    
    # Create rerun list with participant_id and session
    rerun_df["participant_id"] = rerun_df["subject"]
    rerun_df["session"] = "0"
    
    # Select only the two columns we need
    rerun_list = rerun_df[["participant_id", "session"]]
    
    # Write without header and index
    rerun_list.to_csv(rerun_path, sep=",", header=False, index=False)
...
def main():
    ...
    ap.add_argument("res_out", type=str, default="fmriprep_audit.tsv",
                    help="Output table path (.tsv or .csv). Default: fmriprep_audit.csv")
    ap.add_argument("rerun_list", type=str, default=None,
                    help="Path for rerun list output. Default: <same_dir>/rerun_list.csv")
    args = ap.parse_args()
    ...
    # Generate rerun list
    if args.rerun_list:
        rerun_path = Path(args.rerun_list)
    else:
        rerun_path = out_path.parent / "rerun_list.csv"
    
    rerun_file = generate_rerun_list(df, rerun_path)
    if rerun_file:
        print(f"Wrote rerun list: {rerun_file}")
        # Print the rerun list content for verification
        print("\nSubjects to rerun:")
    ...
    print(f"CHECK (needs review/rerun): {len(df[df['status'] != 'OK'])}")
