...
def main(DATA_DIR):
    ...
    if 'PPMI'in DATA_DIR:
        ...
        alldcm = glob.glob(DATA_DIR + '/*/*/*/*/*.dcm')
    elif 'sz_meditation' in DATA_DIR:
        alldcm = glob.glob(DATA_DIR + '/*/*/*/*.dcm')
    elif 'early_late_ocd' in DATA_DIR:
        alldcm = glob.glob(DATA_DIR + '/dicom/*/*/*.dcm')
    else:
        alldcm = glob.glob(DATA_DIR + '/*/*/*/*/*.dcm')
    for jj in range(0,len(alldcm)):
        ds = pydicom.dcmread(alldcm[jj], force=True)
        if jj == 0 :
            if "StudyInstanceUID" in ds.keys():
                studyUID = ds.StudyInstanceUID
            else:
                print("no studyID for the 1st dcm file", alldcm[jj])
                studyUID = "0"
                ds.StudyInstanceUID = studyUID
                ds.save_as(alldcm[jj])
        else:
            if "StudyInstanceUID" not in ds.keys():
                ds.StudyInstanceUID = studyUID
                ds.save_as(alldcm[jj])
            else:
                if ds.StudyInstanceUID != studyUID:
                    print('studyUID conflict or missing detected from', alldcm[jj])
                    ds.StudyInstanceUID = studyUID
                    ds.save_as(alldcm[jj])
    print("StudyUID check finished!")
    return 1
